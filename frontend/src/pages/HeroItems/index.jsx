...
export default function HeroItems() {
  ...
  const title = useMemo(
    () => (heroName ? `Popular Items - ${heroName}` : 'Popular Items'),
    [heroName]
  )
    ...
    async function load() {
      try {
        ...
        const res = await fetch(
          `${API_BASE}/heroes/${heroId}/popular-items?limit=24&playersLimit=10`
        )
            ...
            <div className="players-grid">
              {(data?.top_players || []).map((p) => (
                <div key={p.accountId || p.account_id} className="player-card">
                  ...
                  {p.score != null && (
                    <div className="player-score">Score: {p.score.toFixed(2)}</div>
                  )}
                </div>
              ))}
            </div>
