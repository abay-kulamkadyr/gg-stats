...
public final class LoggingUtils {
	...
	private static final int BATCH_PROGRESS_SMALL_THRESHOLD = 100;

	private static final int BATCH_PROGRESS_MEDIUM_THRESHOLD = 1000;

	private static final int BATCH_PROGRESS_LARGE_INTERVAL = 1000;

	private static final int BATCH_PROGRESS_MEDIUM_INTERVAL = 100;
	...
	public static void logOperationFailure(String operation, String reason, Throwable error, Object... details) {
	...
	private static boolean shouldLogProgress(int current, int total) {
		...
		if (total <= BATCH_PROGRESS_SMALL_THRESHOLD) {
			return current % BATCH_PROGRESS_MEDIUM_INTERVAL == 0; // Every 10% for small
																	// batches
		}
		if (total <= BATCH_PROGRESS_MEDIUM_THRESHOLD) {
			return current % BATCH_PROGRESS_MEDIUM_INTERVAL == 0; // Every 100 for medium
																	// batches
		}
		return current % BATCH_PROGRESS_LARGE_INTERVAL == 0; // Every 1000 for large
																// batches
	}
