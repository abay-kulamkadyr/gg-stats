...
/**
 * Base class for job execution listeners to manage MDC context for the entire job
 * lifecycle. This ensures that MDC context persists across all steps and is only cleared
...
public abstract class BaseJobExecutionListener implements JobExecutionListener {
	...
	public void beforeJob(JobExecution jobExecution) {
		...
		if (correlationId != null) {
			MDCLoggingContext.setBatchContext(jobExecution.getJobInstance().getJobName(),
					jobExecution.getJobId().toString(), null // stepName is null for
																// job-level context
			);
		...
		LoggingUtils.logOperationStart("Starting batch job", "jobName=" + jobExecution.getJobInstance().getJobName(),
				"jobId=" + jobExecution.getJobId(), "correlationId=" + correlationId);
	}
	...
	public void afterJob(JobExecution jobExecution) {
		try {
			if (jobExecution.getStatus().isUnsuccessful()) {
				LoggingUtils.logOperationFailure("Batch job execution", "Job failed",
						new RuntimeException("Job execution failed with status: " + jobExecution.getStatus()),
						"jobName=" + jobExecution.getJobInstance().getJobName(), "jobId=" + jobExecution.getJobId(),
				...
				totalReadCount = jobExecution.getStepExecutions().stream().mapToLong(StepExecution::getReadCount).sum();
				totalWriteCount = jobExecution.getStepExecutions()
					.stream()
				...
				LoggingUtils.logOperationSuccess("Batch job execution",
						"jobName=" + jobExecution.getJobInstance().getJobName(), "jobId=" + jobExecution.getJobId(),
						"correlationId=" + MDCLoggingContext.getCurrentCorrelationId(),
						"exitStatus=" + jobExecution.getExitStatus(), "readCount=" + totalReadCount,
